...
def student_knowledge_mastery(request, student_id):
    if request.method == "GET":
        ...
        if not submit_records.exists():
            return JsonResponse(
                {"error": 1, "msg": "No submit records found for this student"}
            )
        ...
        for know in knowledges:
            ...
            submit_records_knowledge = submit_records.filter(
                title_id__in=DataTitleinfo.objects.filter(
            ...
            submit_records_knowledge = submit_records_knowledge.filter(
                time__in=Subquery(last_record_times)
            )
            ...
            konwledge_list.append(
                {"mainpoint": know.knowledge, "mastery": mastery, "ac_rate": ac_rate}
            )
        # 4. 返回结果
        return JsonResponse({"error": 0, "msg": "success", "data": konwledge_list})
    return JsonResponse({"error": 1, "msg": "Invalid request method"})
    ...
    if request.method == "GET":
        ...
        for sub_know in sub_knowledges:
            ...
            submit_records_sub_knowledge = submit_records.filter(
                title_id__in=DataTitleinfo.objects.filter(
                    sub_knowledge=sub_know.sub_knowledge
                ).values("title_id")
            )
            ...
            # 使用获取到的时间过滤原始的 QuerySet
            submit_records_sub_knowledge = submit_records_sub_knowledge.filter(
                time__in=Subquery(last_record_times)
            )
            # 加权计算该学生对该knowledge的掌握程度
            total_score = 0
        ...
        # 4. 返回结果
        return JsonResponse({"error": 0, "msg": "success", "data": sub_konwledge_list})
    return JsonResponse({"error": 1, "msg": "Invalid request method"})
...
def student_submit_day_record(request, student_id):
    if request.method == "GET":
        ...
        for i in range((end_date - start_date).days + 1):
            ...
            if date not in submit_record_day:
                submit_record_day_list.append({"day": date, "count": 0})
            else:
                submit_record_day_list.append(
                    {"day": date, "count": submit_record_day[date]}
                )

        # 4. 返回结果
        return JsonResponse(
            {"error": 0, "msg": "success", "data": submit_record_day_list}
        )
    return JsonResponse({"error": 1, "msg": "Invalid request method"})
    ...
    if request.method == "GET":
        ...
        for i in range(24):
            if i not in submit_record_hour:
                submit_record_hour_list.append({"period": i, "count": 0})
            else:
                submit_record_hour_list.append(
                    {"period": i, "count": submit_record_hour[i]}
                )

        # 4. 返回结果
        return JsonResponse(
            {"error": 0, "msg": "success", "data": submit_record_hour_list}
        )
    return JsonResponse({"error": 1, "msg": "Invalid request method"})
...
def student_submit_record(request, student_id):
    if request.method == "GET":
        ...
        for record in submit_records:
            ...
            knows = DataTitleinfo.objects.filter(title_id=record.title_id).values_list(
                "knowledge", flat=True
            )
            sub_knows = DataTitleinfo.objects.filter(
                title_id=record.title_id
            ).values_list("sub_knowledge", flat=True)
            for know in knows:
        ...
        for method in method_count:
            node_list.append({"name": method})
            link_list.append(
                {"source": "total", "target": method, "value": method_count[method]}
            )
        for know in knowledge_submit_count:
            node_list.append({"name": know})
        for sub_know in sub_knowledge_submit_count:
            node_list.append({"name": sub_know})
            link_list.append(
                {
...
def submit_knowledge(request):
    if request.method == "GET":
        ...
        return JsonResponse({"error": 0, "msg": "success", "data": knowledge_list})
    return JsonResponse({"error": 1, "msg": "Invalid request method"})
...
def student_submit_grade(request, student_id):
    if request.method == "GET":
        ...
        submit_records_dict = {
            record.title_id: record for record in submit_records.order_by("-time")
        }
        ...
        for title in titles:
            ...
            if key in maps:
                ...
                maps[key] = 1
        count1.append(
            {"value": ac_list[0], "name": "已通过", "itemStyle": {"color": "#009E05"}}
        )
        count1.append(
            {
                "value": trying_list[0],
                "name": "尝试中",
                "itemStyle": {"color": "#2052BD"},
            }
        )
        count1.append(
            {
        ...
        switcher = {1: "简单", 2: "中等", 3: "困难"}
        for i in range(3, 0, -1):
            count2.append(
                {
                    "value": ac_list[i],
                    "name": "已通过-" + switcher.get(i),
                    "itemStyle": {"color": getColor("已通过-" + switcher.get(i))},
                }
            )
        for i in range(3, 0, -1):
            count2.append(
                {
                    ...
                    "itemStyle": {"color": getColor("尝试中-" + switcher.get(i))},
                }
            )
        for i in range(3, 0, -1):
            count2.append(
                {
                    ...
                    "itemStyle": {"color": getColor("未提交-" + switcher.get(i))},
                }
            )
        ...
        return JsonResponse(
            {
                "error": 0,
                "msg": "success",
                "data": {"count1": count1, "count2": count2, "count3": count3},
            }
        )
...
def student_knowledge_1(request):
    if request.method == "POST":
        ...
        submit_records = submit_records.filter(
            title_id__in=DataTitleinfo.objects.filter(knowledge=knowledge).values(
                "title_id"
            )
        )
        if not submit_records.exists():
            return JsonResponse(
                {"error": 1, "msg": f"No records found for knowledge point {knowledge}"}
            )
    ...
    if request.method == "POST":
        ...
        if not submit_records.exists():
            return JsonResponse(
                {
                    "error": 1,
                    "msg": f"No submit records found for knowledge {knowledge}",
                }
            )
        # 对于submit_records中的每一条记录，如果有title_id相同的，保留最后一次的提交记录
...
def student_knowledge_distribution(request):
    if request.method == "POST":
        ...
        for record in submit_records:
            knowledge_records = DataTitleinfo.objects.filter(
                title_id=record.title_id
            ).values_list("knowledge", flat=True)
        ...
        for record in submit_records:
            sub_knowledge_records = DataTitleinfo.objects.filter(
                title_id=record.title_id
            ).values_list("sub_knowledge", flat=True)
